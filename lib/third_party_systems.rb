...
=end
  ...
  def self.get_lnt_product_variations(options={})
    ...
    if options[:new_only]
      product_variations = ProductVariation.find(:all, :conditions => ['products.visible = ? AND products.include_in_feeds = ? AND product_variations.created_at >= ?', true, true, 7.days.ago.beginning_of_day], :include => [:product])
      ...
      product_variations = ProductVariation.find(:all, :conditions => ['products.visible = ? AND products.include_in_feeds = ?', true, true], :include => [:product])
  ...
  def self.get_lnt_price(variation)
    ...
    lnt_sale.each do|item|
      if variation.product_number == item.split(",").first
        return item.split(",").last.to_money
      end
    end
